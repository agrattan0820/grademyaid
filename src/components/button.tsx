...
const Button = ({ color, label, icon, onClick, outline, size }: Props) => {
  return (
    <button
      onClick={onClick}
      className={`flex h-full min-w-[100px] items-center justify-center rounded-full font-bold transition focus-visible:ring-2 md:hover:scale-105 
      ${
        size === "small"
          ? "px-2 py-1 text-sm"
